...
function watch(source, callback) {
  // TODO: no-op to satisfy theA API
}
class EditorCache {
  editorCache = $state({
    lru: [],
  ...
  moveCurrentBlockToOtherEditor(targetPath) {
    ...
    const editor = this.getEditor(heynoteStore.currentBufferPath);
    let otherEditor = this.getOrCreateEditor(targetPath, false);
    otherEditor.hide();
...
let editorCache = new EditorCache();

export function useEditorCacheStore() {
  return editorCache;
}
